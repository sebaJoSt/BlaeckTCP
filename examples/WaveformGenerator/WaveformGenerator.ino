/*
  ...
  A dashboard-friendly demo: BlaeckTCP -> Loggbok (TCP host and MQTT bridge) ->
  MQTT broker -> Home Assistant (MQTT client and dashboard).

  Also the example that takes updates over the network and announces itself with Bonjour,
  on the Mega and the Giga with an Ethernet shield and on the ESP32-PoE. Each board needs a
  one-time setup for that first: see README.md in this folder.
