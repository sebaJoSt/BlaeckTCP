// Globals the @code blocks in BlaeckTCP.h and BlaeckCore.h are written against.
// Only one file includes this, and it does not exist until it is generated: DocCodeBlocks.ino
//
// Mirrors BlaeckSerial's preamble, so the core's blocks compile in both libraries. Add a
// name there first unless only BlaeckTCP's own blocks use it.
...
// The port every BlaeckTCP example listens on.
...
inline void onClientConnected(byte clientNo) {}
inline void onClientDisconnected(byte clientNo) {}
